...
class Charts(object):
    ...
    def ban_time(self):
        ...
        plt.savefig('./img/ban_time.png', format='png', dpi=400)
        plt.close()
    ...
    def speak_photo_in_total(self):
        ...
        plt.savefig('./img/speak_photo_in_total.png', format='png', dpi=300)
        plt.close()
    ...
    def user_online_time(self, user_ID=None):
        ...
        plt.savefig('./img/user_time_online_test.png', format='png', dpi=300)
        plt.close()

    def speak_bin(self):
        ind = Individual()
        res_list = ind.most_speak('speak_num')
        data =[x[2] for x in res_list]
        name = [x[1] for x in res_list]
        print(data,name)
        plt.figure(figsize=(18.5, 9))
        plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定默认字体
        plt.pie(data,labels=name, startangle = 90,
        counterclock = False, wedgeprops = {'width' : 0.4})
        plt.axis('square')
        plt.savefig('./img/pie_test.png', format='png', dpi=300)
        plt.close()
